// ============================================================
// LiteCNN Cycle 13 LibTorch MPS Server (Port 8893)
// Target: <10ms inference with M1 GPU acceleration
// Version 3: Korean/English breed names support
// ============================================================
...
torch::Tensor preprocess_image_from_file(const string& image_path, double& preprocess_ms) {
    auto start = high_resolution_clock::now();
...
int main() {
    cout << "============================================================\n";
    cout << "LiteCNN Cycle 13 LibTorch MPS Server v3\n";
    cout << "Port: 8893 | Target: <10ms inference\n";
    cout << "Features: KO/EN breed names, Multipart upload\n";
    cout << "============================================================\n\n";
    ...
    // Load model
    cout << "📦 Loading model: weights/cycle13_mps_traced.pt\n";
    try {
        model = torch::jit::load("weights/cycle13_mps_traced.pt", device);
        model.eval();
    ...
    svr.Post("/predict", [](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            // Check if multipart image upload
            if (req.form.has_file("image")) {
                ...
                // Check if top_k is in multipart params
                if (req.has_param("top_k")) {
                    top_k = stoi(req.get_param_value("top_k"));
                }
            }
            // Check if JSON body with image_path
            else if (!req.body.empty()) {
                auto body = json::parse(req.body);
                string image_path = body["image_path"];
                top_k = body.value("top_k", 3);
                
                input_tensor = preprocess_image_from_file(image_path, preprocess_ms);
            }
            else {
                throw runtime_error("No image provided (use multipart 'image' or JSON 'image_path')");
            }
    ...
    svr.Get("/health", [](const httplib::Request&, httplib::Response& res) {
        json health;
        health["status"] = "ok";
        health["model"] = "Cycle 13 (Epoch 27, 62.60%)";
    ...
    cout << "\n🚀 Server starting on http://0.0.0.0:8893\n";
    cout << "============================================================\n";
    
    svr.listen("0.0.0.0", 8893);
